...
mod tests {
  ...
  #[test]
  fn test_preserves_subscript_and_superscript() {
    let input = "<p>H<sub>2</sub>O and x<sup>2</sup></p>";
    let output = convert_to_markdown(input);
    assert!(output.contains("H<sub>2</sub>O"), "{output:?}");
    assert!(output.contains("x<sup>2</sup>"), "{output:?}");
  }
...
line</p>
  ...
  #[test]
  fn test_styled_span_preserves_nested_formatting() {
    let input = r#"<p><span style="color: rgb(7, 71, 166);"><del>a</del>H<sub>2</sub>O<sup>+</sup></span></p>"#;
    let output = convert_to_markdown(input);
    assert!(
